...
impl Remote {
    pub(in crate::api) fn new(addr: SocketAddr, client: Arc<dyn http_client::Client + Send + Sync + 'static>) -> Self {
        Self { addr, client }
    }

    fn validate_request_requirements(&self, requirements: &RequestRequirements) -> Result<(), Error> {
        match requirements.is_true {
            Some(_) => Err(Error::InvalidMockDefinition(
    ...
    fn build_request(&self, method: &str, path: &str, json_body: Option<String>) -> Result<Request<Bytes>, Error> {
        ...
        builder.body(body).map_err(|e| Error::Upstream(e.to_string()))
    }

    /// Sends a request and returns the response body as a string, failing with
    /// an [`Error::Upstream`] when the status does not match `expected_status`.
    ...
    async fn do_request(&self, req: Request<Bytes>) -> Result<(u16, String), Error> {
        ...
        let body = String::from_utf8(body_bytes.to_vec()).map_err(|e| Error::Upstream(e.to_string()))?;
...
impl Adapter for Remote {
    ...
    async fn create_mock(&self, mock: &MockDefinition) -> Result<ActiveMock, Error> {
        ...
        let json = serde_json::to_string(mock).map_err(Error::JsonSerialization)?;
    ...
    async fn verify(&self, requirements: &RequestRequirements) -> Result<Option<ClosestMatch>, Error> {
        let json = serde_json::to_string(requirements).map_err(Error::JsonSerialization)?;
        ...
        let response: ClosestMatch = serde_json::from_str(&body).map_err(Error::JsonDeserialization)?;
    ...
    async fn create_forwarding_rule(&self, config: ForwardingRuleConfig) -> Result<ActiveForwardingRule, Error> {
        ...
        let json = serde_json::to_string(&config).map_err(Error::JsonSerialization)?;
