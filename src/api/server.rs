...
pub struct MockServer {
    pub(in crate::api) server_adapter: Option<Arc<dyn backend::Adapter + Send + Sync>>,
    pool: Arc<Pool<Arc<dyn backend::Adapter + Send + Sync>>>,
}

impl MockServer {
    async fn from(
        server_adapter: Arc<dyn backend::Adapter + Send + Sync>,
        pool: Arc<Pool<Arc<dyn backend::Adapter + Send + Sync>>>,
    ) -> Self {
        let server = Self {
    ...
    pub async fn connect_async(address: &str) -> Self {
        ...
        let adapter = REMOTE_SERVER_POOL_REF
            .take_or_create(|| Arc::new(backend::Remote::new(addr, REMOTE_SERVER_CLIENT.clone())))
            .await;
...
const LOCAL_SERVER_ADAPTER_GENERATOR: fn() -> Arc<dyn backend::Adapter + Send + Sync> = || {
    ...
    Arc::new(backend::Local::new(addr, state_manager))
};

static LOCAL_SERVER_POOL_REF: LazyLock<Arc<Pool<Arc<dyn backend::Adapter + Send + Sync>>>> = LazyLock::new(|| {
...
static REMOTE_SERVER_POOL_REF: LazyLock<Arc<Pool<Arc<dyn backend::Adapter + Send + Sync>>>> =
...
static REMOTE_SERVER_CLIENT: LazyLock<Arc<http_client::HyperClient>> = LazyLock::new(|| {
    ...
    Arc::new(http_client::HyperClient::new(Some(Arc::new(
